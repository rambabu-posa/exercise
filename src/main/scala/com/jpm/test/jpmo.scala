...
object Jpmo extends App {
  ...
  // analytics.rankStationsByOnlinePerMonth(df,schemaConf,defaults)
  // analytics.rankStationsByRainfall(df,defaults)
  // analytics.rankStationsBySunshine(df,defaults)
   // analytics.worstRainfall(df,defaults)
   // analytics.avgRainfall(df,defaults)
  // analytics.bestSunshinefall(df,defaults)
  ...
  //analytics.averagesAcrossSun(df,schemaConf,defaults)
   analytics.yearWiseMetrics(df,schemaConf,defaults)
